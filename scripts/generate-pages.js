...
const { contact, nav, services, projects, process, insights, jobs, testimonials, team } = require("../data/site-data");

const root = path.resolve(__dirname, "..");
const cacheKey = "multi-page-20260623";
const brochure = "public/IDH-brochure-placeholder.pdf";
...
function footer() {
  ...
  const socials = Object.entries(contact.socials)
    .map(([label, href]) => `<a href="${href}" ${href === "#" ? "" : "target=\"_blank\" rel=\"noopener noreferrer\""} aria-label="${label}">${label}</a>`)
    .join("");
  return `
  <footer class="site-footer">
    ...
    <div class="footer-bottom footer-container">
      <p>© 2025 Integrated Design Habitat (IDH). All rights reserved. <a href="privacy-policy.html">Privacy Policy</a></p>
...
const serviceCards = (items = services) =>
  items.map((service) => `
    <article class="lux-card reveal">
      ...
      <h3>${service.title}</h3>
      <p>${service.excerpt}</p>
      <a href="contact.html">Explore <i>→</i></a>
    </article>`).join("");
  ...
  return `
    <form class="lux-form reveal" data-mail-form data-recipient="${target}" data-subject="${isCareer ? "Career Application" : isSales ? "Sales Enquiry / Request a Quote" : "General Enquiry"}">
      <div class="form-grid">
        ...
        <label class="full">CV upload placeholder<input type="file" name="CV upload placeholder" disabled /><small>Static placeholder — connect to a form backend when ready.</small></label>` : `
...
function home() {
  ...
  <main>
    <section class="hero" id="home" aria-labelledby="hero-title">
      ...
      <a class="scroll-cue" href="#clients"><span>Scroll to explore</span><b>↓</b></a>
    </section>
    <section class="selected-clients" id="clients" aria-labelledby="clients-title">
      <div class="clients-intro reveal"><p class="eyebrow">Selected Clients</p><i></i><h2 id="clients-title">Trusted by visionary brands<br />and discerning partners.</h2><p>We collaborate with forward-thinking clients to shape<br />spaces that inspire and endure.</p></div>
      <div class="client-grid reveal" aria-label="Selected client list">
        ${["Atelier One|Design Studio","Noor|Holdings","Stonecrest|Developments","Meridian|Properties","Al Raya|Group","Horizon|Investments","Aurelia|Homes","Habitat|Living","Vesta|Capital","Lumen|Collection"].map((item, index) => {
          const [name, sub] = item.split("|");
          return `<div class="client-mark${index % 3 === 1 ? " gold" : ""}"><strong>${name}</strong><small>${sub}</small></div>`;
        }).join("")}
      </div>
      <div class="clients-link reveal"><i></i><a href="projects.html">View Collaborations <span>⟶</span></a><i></i></div>
    </section>
    <section class="about-feature" id="about">
      <div class="about-scene reveal"><img src="architecture_placeholders_webp/05-about-studio-vignette.webp" alt="Atmospheric crafted interior by Integrated Design Habitat" loading="lazy" /><div class="about-panel"><div class="about-label"><p class="eyebrow">About the studio</p><i></i></div><h2>Design-Led.<br />Detail-Driven.<br />Purposeful Spaces.</h2><p>We create timeless environments shaped by intention, craft and a quiet commitment to quality.</p><p>Every project begins with its people and ends with a space that feels entirely their own.</p><a href="about.html">Learn more about us <span>›</span></a></div></div>
    ...
    <section class="section projects-section" id="projects"><div class="section-heading reveal"><div><p class="eyebrow">Featured projects</p><h2>Spaces Crafted With Intention</h2></div><a class="text-link" href="projects.html">View all projects →</a></div><div class="project-grid">${projectCards(projects.slice(0, 4))}</div></section>
    <section class="section process-showcase" id="process"><div class="process-heading reveal"><div><p class="eyebrow">Our process</p><h2>A Collaborative<br />&amp; Considered Approach</h2></div><i><b></b></i><p>Clarity, creativity and confidence at every stage—from first conversation to final detail.</p></div><div class="timeline-grid">${process.map(([num, title, text]) => `<article><span>${num}</span><h3>${title}</h3><p>${text}</p></article>`).join("")}</div></section>
    <section class="section journal" id="journal"><div class="section-heading reveal"><div><p class="eyebrow">Insights</p><h2>Journal</h2></div><a class="text-link" href="insights.html">View all articles →</a></div><div class="journal-grid">${insights.slice(0, 2).map((post) => `<article class="reveal"><img src="${post.image}" alt="${esc(post.title)}" loading="lazy" /><div><small>${post.date} · ${post.category}</small><h3>${post.title}</h3><a href="insights.html">Read more →</a></div></article>`).join("")}</div></section>
...
function about() {
  ...
  <main>
    ...
    <section class="content-section messages"><article class="reveal"><p class="eyebrow">Founder Message</p><h2>“Quiet design has the power to change how we live.”</h2><p>This placeholder message will introduce the founder’s design philosophy, studio values and belief in meaningful client collaboration.</p></article><article class="reveal"><p class="eyebrow">Co-founder Message</p><h2>“The strongest spaces are built from listening first.”</h2><p>This placeholder message will introduce the co-founder’s role in shaping design direction, process and project delivery.</p></article></section>
    ...
    <section class="content-section values-grid">${["Context before style","Purposeful luxury","Material honesty","Clear collaboration","Enduring details","Responsible decisions"].map((value) => `<article class="reveal"><span>◇</span><h3>${value}</h3><p>Placeholder value copy that explains how this principle shapes IDH’s work and client experience.</p></article>`).join("")}</section>
  </main>`);
}
...
function servicesPage() {
  ...
  <main>
    ...
    <section class="content-section"><div class="section-heading reveal"><div><p class="eyebrow">Our Disciplines</p><h2>Design Services</h2></div>${brochureButton}</div><div class="detail-grid">${services.map((service) => `<article class="detail-card reveal"><img src="${service.image}" alt="${esc(service.title)}" loading="lazy" /><span>${service.number}</span><h3>${service.title}</h3><p>${service.detail}</p><a href="contact.html">Discuss this service →</a></article>`).join("")}</div></section>
    <section class="contact-cta"><p class="eyebrow">Brochure</p><h2>Explore the IDH service overview.</h2><p>Download the placeholder brochure now and replace it with the final studio PDF when ready.</p><div class="cta-actions">${brochureButton}<a class="button button-gold" href="contact.html">Request a Quote</a></div></section>
  </main>`);
}
...
function projectsPage() {
  ...
  <main>
    ${pageHero("Projects", "Selected spaces crafted with intention.", "A placeholder portfolio of residential, commercial, hospitality and spatial design work by IDH.", "architecture_placeholders_webp/06-project-private-residence.webp")}
...
function processPage() {
  ...
  <main>
    ${pageHero("Process", "A collaborative and considered approach.", "Clarity, creativity and confidence at every stage—from first conversation to final detail.", "architecture_placeholders_webp/04-hero-moody-interior-corner.webp")}
...
function insightsPage() {
  ...
  <main>
    ${pageHero("Insights", "Announcements, news, articles and studio notes.", "A flexible journal structure for IDH’s updates, ideas and video-led stories.", "architecture_placeholders_webp/10-insight-designing-wellbeing.webp")}
    ...
    <section class="content-section insight-grid">${insights.map((post) => `<article class="insight-card reveal" id="${post.category.toLowerCase()}"><img src="${post.image}" alt="${esc(post.title)}" loading="lazy" /><div><span>${post.category}</span><small>${post.date}</small><h3>${post.title}</h3><p>${post.excerpt}</p>${post.youtube ? `<div class="video-embed"><iframe src="${post.youtube}" title="${esc(post.title)} video preview" loading="lazy" allowfullscreen></iframe></div>` : ""}<a href="#">Read more →</a></div></article>`).join("")}</section>
  </main>`);
}
...
function contactPage() {
  ...
  <main>
    ...
    <section class="content-section contact-panels"><article class="reveal"><p class="eyebrow">General</p><h2>Studio Contact</h2><p><a href="mailto:${contact.email}">${contact.email}</a><br /><a href="tel:${contact.phoneHref}">${contact.phone}</a><br />${contact.address}</p><div class="social-row">${Object.entries(contact.socials).map(([label, href]) => `<a href="${href}" ${href === "#" ? "" : "target=\"_blank\" rel=\"noopener noreferrer\""}>${label}</a>`).join("")}</div></article><article class="reveal"><p class="eyebrow">WhatsApp</p><h2>Quick Actions</h2><div class="cta-actions stacked"><a class="button button-gold" href="${whatsappHref("Hello IDH, I would like to book a design consultation.")}" target="_blank" rel="noopener noreferrer">Book Consultation</a><a class="button button-outline" href="${whatsappHref("Hello IDH, I would like to request a quote for my project.")}" target="_blank" rel="noopener noreferrer">Request a Quote</a><a class="button button-outline" href="careers.html">Career Enquiry</a></div></article></section>
    ...
    <section class="content-section map-section"><div class="section-heading reveal"><div><p class="eyebrow">Visit</p><h2>Find the Studio</h2></div></div><iframe title="IDH Google Map placeholder" loading="lazy" src="https://www.google.com/maps?q=Maas%20Tower%20Ground%20Floor%20Manjeri%20Malappuram%20Kerala&output=embed"></iframe></section>
  </main>`);
}
...
function privacyPage() {
  ...
  <main>
    ${pageHero("Privacy Policy", "A clear note on how IDH handles enquiries.", "This placeholder policy can be reviewed by the client or legal advisor before launch.", "architecture_placeholders_webp/09-insight-natural-materials.webp")}
    <section class="content-section policy-copy reveal">
      ...
      <p>This static website currently opens the visitor’s email client for submissions. If a database or third-party form service is connected later, this policy should be updated to describe that service.</p>
